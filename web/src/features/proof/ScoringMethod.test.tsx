...
vi.mock("../../lib/api", async (orig) => ({
  ...
  getSettings: vi.fn(async () => ({ sandbox_enabled: false, powermetrics_gpu_optin: false, provider_max_retries: 2, thresholds: { similarity: 0.55, keypoint: 0.8, judge: 0.8 } }) as Settings),
}));
...
describe("ScoringMethod", () => {
  beforeEach(() => {
    ...
    vi.mocked(getSettings).mockResolvedValue({ sandbox_enabled: false, powermetrics_gpu_optin: false, provider_max_retries: 2, thresholds: { similarity: 0.55, keypoint: 0.8, judge: 0.8 } } as Settings);
  });
  ...
  it("Sandbox ON + sample dataset: does NOT auto-select the Mock judge (stays on the keyless demo)", async () => {
    vi.mocked(getSettings).mockResolvedValue({ sandbox_enabled: true, powermetrics_gpu_optin: false, provider_max_retries: 2, thresholds: { similarity: 0.55, keypoint: 0.8, judge: 0.8 } } as Settings);
    const onChange = vi.fn();
    render(wrap(<ScoringMethod value={null} onChange={onChange} dataset={sampleDataset} />));
    const judge = await screen.findByRole("button", { name: /LLM judge/i });
