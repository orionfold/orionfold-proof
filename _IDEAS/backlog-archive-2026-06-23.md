# _IDEAS — Backlog ARCHIVE (frozen 2026-06-23)

> **FROZEN SNAPSHOT.** This is the full B1–B8 reasoning at the moment the backlog was
> merged into `HANDOFF.md` and `_IDEAS/backlog.md` was reset for fresh intake. Do NOT add
> new items here — new intake goes in `_IDEAS/backlog.md`. The live prioritized list lives
> in `HANDOFF.md` ("MERGED BACKLOG — prioritized"); this file is the durable detail store
> the HANDOFF table points back to for per-item analysis (Arena reuse notes, migration
> steps, invariant forks).
>
> **Priority mapping applied at merge (HANDOFF row → archive item):**
> 1 B6 (NOW/HIGHEST · ADRs 0004-0006 already written; brainstorm+slice remain) ·
> 2 B7 (HIGH · blocks #8; dirs confirmed still real, not symlinks) ·
> 3 #7 packaging (downstream of B6) · 4 B4 (PAUSED on B6; web screen shipped 33339d5) ·
> 5 B5 · 6 cross-product models×prompts · 7 catalog price pass · 8 B2 · 9 DS-skin polish ·
> 10 B8 · 11 B1 · 12 recommended-on-0/5 backfill · 13 richer sample data ·
> 14 #8 git remote+push (LAST, gated on B6→#7 AND B7). ✅ B3 SHIPPED (af8203d), dropped.

---

# _IDEAS — Backlog (someday / low-priority) — historical content below
